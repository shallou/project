     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      <thead> 
       <tr role="row">
        ...
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 179px;">轮播图</th>
      ...
      <tbody role="alert" aria-live="polite" aria-relevant="all">
        @foreach($info as $v)
       <tr class="odd"> 
        <td class="  sorting_1">{{$v->bid}}</td> 
        <td class=" ">{{$v->name}}</td> 
        <td class=" ">第{{$v->banner_id}}张轮播图</td> 
        
        <td class=" "><img src="{{$v->banner}}" width="100px" height="100px"></td> 

        <td class=" ">
          <form action="/banner/{{$v->bid}}" method="post">
            <button class="btn btn-success" type="submit"><i class="icon-trash"></i></button>
            {{csrf_field()}}
            {{method_field('DELETE')}}
          </form>
          <a href="/banner/{{$v->bid}}/edit"class="btn btn-info"><i class="icon-wrench"></i></a></td> 
       </tr>
       @endforeach
      </tbody>
     </table>
